...
app.get("/api/notion/data", async (req, res) => {
  ...
  try {
    ...
    const brandingChildren = brandingPage ? pages.filter((p: any) => p.parentId === brandingPage.id && !p.title.toLowerCase().includes('manual de marca_compressed') && !p.title.toLowerCase().includes('manual de marca compressed')) : [];
